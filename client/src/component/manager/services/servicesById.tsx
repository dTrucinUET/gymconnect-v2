...
const ServiceManagementById = (props: any) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:8080/service?roomid=${id}`, {
                method: 'GET',
                headers: {
            ...
            showNotification({
                title: 'Error',
                message: 'Có vấn đề gì với server này!',
                color: 'red',
                position: 'bottom-right',
            });
        }
    ...
    const confirmDelete = async () => {
        if (serviceIdDelete) {
            try {
                const response = await fetch(`http://localhost:8080/service/${serviceIdDelete}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${user.token}`
    ...
    const handleSubmitEditService = async (updatedData: Service) => {
        try {
            const response = await fetch(`http://localhost:8080/service/${updatedData.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
...
export default ServiceManagementById;
