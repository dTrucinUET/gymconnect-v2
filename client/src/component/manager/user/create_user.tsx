...
interface AdminUserCreationModalProps {
    opened: boolean;
    onClose: () => void;
    onCreateUser: (userData: any, roomid: any) => void;
    roomList: Number[];
}

const AdminUserCreationModal: React.FC<AdminUserCreationModalProps> = ({ opened, onClose, onCreateUser, roomList }) => {
        ...
        <Modal opened={opened} onClose={onClose} title="Create New User" size="lg">
            <form onSubmit={form.onSubmit(handleSubmit)}>
                ...
                <Select
                    ...
                    data={Array.from(new Set(roomList)).map((room) => ({
                        value: room.toString(), // Giá trị của từng checkbox
                        label: `Room ${room}`, // Nhãn hiển thị
                    }))}
                    onChange={(value) => {
                        const selectedRoomId = Number(value); // Chuyển giá trị thành số
                        setRoomId(selectedRoomId); // Cập nhật state
                    }}
                    mb="md"
                />
                <Group>
