...
import AdminUserCreationModal from './create_user';
...
const UserManagement = () => {
    ...
    const [roomIdListInUsers, setRoomIdListInUsers] = useState<Number[]>([])
    // const fetchData = async () => {
    //     try {
    //         console.log('fetch user list in manager');
    ...
    //         const response_roomId = await fetch('http://localhost:8080/user_room', {
    //             method: 'GET',
    //             headers: {
    ...
    //             credentials: 'include',
    //         });
    //         if (response_roomId.status === 200) {


    //             const response = await fetch('http://localhost:8080/user_room?roomid=1', {
    //                 method: 'GET',
    //                 headers: {
    //                     'Content-Type': 'application/json',
    //                     'Authorization': `Bearer ${user.token}`
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch all user_room data
            const response_userRoom = await fetch('http://localhost:8080/user_room', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
            ...
            if (response_userRoom.status === 200) {
                const userRoomData = await response_userRoom.json();
                console.log(userRoomData);
                let userId = user.id;
                let roomIdList: any[] = [];
                for (let i = 0; i < userRoomData.length; i++) {
                    if (userRoomData[i].user_id === userId) {
                        roomIdList.push(userRoomData[i].room_id);
                    }
                }
                setRoomIdListInUsers(roomIdList)
                console.log(roomIdList);
                let userAllData: User[] = [];
                await Promise.all(
                    roomIdList.map(async (roomId: number) => {
                        try {
                            const response_userRoom = await fetch(`http://localhost:8080/user_room?roomid=${roomId}`, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${user.token}`,
                            ...
                            if (!response_userRoom.ok) throw new Error(`Error: ${response_userRoom.status}`);
                            const data = await response_userRoom.json();
                ...
                console.log('user nè', userAllData);
                const uniqueUsers = userAllData.filter(
                    (user, index, self) =>
                        index === self.findIndex(u => u.id === user.id)
                );
                setDataUser(uniqueUsers);
    ...
    const handleSubmitCreateUser = async (userCreate: any, roomId: any) => {
        console.log('userCreate in Admin', userCreate, roomId);
        ...
        console.log("user room", userCreate.id, roomId);
        ...
        const response2 = await fetch('http://localhost:8080/user_room', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${user.token}`
    ...
    const handleRoleChange = async (userId: number, roleId: number) => {
        try {
            console.log('handleRoleChange in Manager', userId, roleId);
